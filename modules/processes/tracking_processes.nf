...
process GENERATE_MASKS {
    ...
    input:
        tuple val(sid), path(brain_mask), path(wm_mask), path(fa)
    output:
    ...
    scil_image_math.py intersection wm_mask_temp.nii.gz $brain_mask\
        ${sid}__wm_mask_final.nii.gz --data_type uint8 -f
    scil_image_math.py convert $brain_mask ${sid}__brain_mask.nii.gz\
        --data_type uint8 -f
    """
}
...
process LOCAL_TRACKING {
    ...
    script:
    if (params.use_brain_mask_as_tracking_mask)
    """
    export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1
    export OMP_NUM_THREADS=1
    export OPENBLAS_NUM_THREADS=1
    scil_compute_local_tracking.py $fodf $wm_mask $brain_mask\
